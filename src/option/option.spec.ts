    ...
    describe('typeguards', () => {
        ...
        test('isSome', () => {
            if (some.isSome()) {
                ...
                const _mappedSome = some.map(val => val.length);
                type mappedUnwrapRes = Equals<number, ReturnType<typeof _mappedSome.unwrap>>;
        ...
        test('isNone', () => {
            if (none.isNone()) {
                ...
                const _mappedNone = none.map(val => val.length);
                type mappedUnwrapRes = Equals<never, ReturnType<typeof _mappedNone.unwrap>>;
                type _mappedUnwrapRes = Expect<mappedUnwrapRes>;
            }
        });
    });
