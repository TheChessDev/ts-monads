 ...
 * ```
 */
export function isOk<T extends NonUndefined, E extends NonUndefined>(val: Result<T, E>): val is OkResult<T, never> {
    return val.isOk();
}
 ...
 * ```
 */
export function isErr<T extends NonUndefined, E extends NonUndefined>(val: Result<T, E>): val is ErrResult<never, E> {
    return val.isErr();
}
