    ...
    describe('typeguards', () => {
        ...
        test('isOk', () => {
            if (ok.isOk()) {
                ...
                const _mappedOk = ok.map(val => val.length);
                type mappedUnwrapOkRes = Equals<number, ReturnType<typeof _mappedOk.unwrap>>;
        ...
        test('isErr', () => {
            if (err.isErr()) {
                ...
                const _mappedErr = err.mapErr(val => val.length);
                type mappedUnwrapErrRes = Equals<number, ReturnType<typeof _mappedErr.unwrapErr>>;
