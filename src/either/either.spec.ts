    ...
    describe('typeguards', () => {
        ...
        test('isLeft', () => {
            if (left.isLeft()) {
                ...
                const _mappedLeft = left.mapLeft(val => val.length);
                type mappedUnwrapLeftRes = Equals<number, ReturnType<typeof _mappedLeft.unwrap>>;
                type _mappedUnwrapLeftRes = Expect<mappedUnwrapLeftRes>;

                const _mappedRight = left.mapRight(() => 42);
                type mappedUnwrapRightRes = Equals<'error' | number, ReturnType<typeof _mappedRight.unwrap>>;
        ...
        test('isRight', () => {
            if (right.isRight()) {
                ...
                const _mappedLeft = right.mapLeft(() => 42);
                type mappedUnwrapLeftRes = Equals<'success' | number, ReturnType<typeof _mappedLeft.unwrap>>;
                type _mappedUnwrapLeftRes = Expect<mappedUnwrapLeftRes>;

                const _mappedRight = right.mapRight(val => val.length);
                type mappedUnwrapRightRes = Equals<number, ReturnType<typeof _mappedRight.unwrap>>;
                type _mappedUnwrapRightRes = Expect<mappedUnwrapRightRes>;
            }
        });
    });
