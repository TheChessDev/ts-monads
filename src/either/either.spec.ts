  ...
  describe("typeguards", () => {
    ...
    test("isLeft", () => {
      if (left.isLeft()) {
        ...
        type unwrapLeftRes = Equals<
          "error",
          ReturnType<typeof left.unwrapLeft>
        >;
        type _unwrapLeftRes = Expect<unwrapLeftRes>;

        type unwrapRightRes = Equals<
          never,
          ReturnType<typeof left.unwrapRight>
        >;
        type _unwrapRightRes = Expect<unwrapRightRes>;

        const mappedLeft = left.mapLeft((val) => val.length);
        type mappedUnwrapLeftRes = Equals<
          number,
          ReturnType<typeof mappedLeft.unwrap>
        >;
        type _mappedUnwrapLeftRes = Expect<mappedUnwrapLeftRes>;

        const mappedRight = left.mapRight(() => 42);
        type mappedUnwrapRightRes = Equals<
          "error" | number,
          ReturnType<typeof mappedRight.unwrap>
        >;
    ...
    test("isRight", () => {
      if (right.isRight()) {
        type unwrapRes = Equals<
          "success",
          ReturnType<typeof right.unwrapRight>
        >;
        type _unwrapRes = Expect<unwrapRes>;
        ...
        type unwrapRightRes = Equals<
          "success",
          ReturnType<typeof right.unwrap>
        >;
        type _unwrapRightRes = Expect<unwrapRightRes>;
